...
namespace Utility.任务
{
    ...
    public class H优先级队列<TValue> : IProducerConsumerCollection<KeyValuePair<int, TValue>>
    {
    ...
    public class Test_H优先级队列
    {
        public static void Main()
        {
            int priorityCount = 7;
            var queue = new H优先级队列<int>(priorityCount);
            ...
            tasks[1] = Task.Factory.StartNew(() =>
            {
                while (!bc.IsCompleted && !cts.Token.IsCancellationRequested)
                {
                    ...
                    var item = new KeyValuePair<int, int>();
                    bool success = false;
